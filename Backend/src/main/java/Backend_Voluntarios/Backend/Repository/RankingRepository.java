...
public interface RankingRepository extends JpaRepository<RankingEntity, Long> {
    ...
    @Transactional
    @Modifying
    @Query(value = "INSERT INTO Entity_Voluntario (Long idTarea, Long idVoluntario, Integer nivelRanking, String tareaRanking) " +
            "VALUES (:idTarea, :idVoluntario, :nivel, :tarea)")
    void crearRanking(@Param("idTarea") Long idTarea,
    ...
    @Query("SELECT COUNT(DISTINCT v.equipamientoVoluntario) FROM VoluntarioEntity v " +
            "INNER JOIN TareaHabilidadEntity t ON v.equipamientoVoluntario = t.habilidadRequerida " +
            "WHERE v.idVoluntario = :idVoluntario AND t.tarea.idTarea = :idTarea")
    ...
    public int matchHabilidad(@Param("id") Long idVoluntario);
}